...
def slide_collate_fn_two_views(samples):
    """Collate for two-view supervised pretraining.
    Returns dict with padded imgs/coords for both views and labels.
    """
    imgs1 = [s["imgs1"] for s in samples]
    coords1 = [s["coords1"] for s in samples]
    imgs2 = [s["imgs2"] for s in samples]
    coords2 = [s["coords2"] for s in samples]
    labels = torch.stack([s["labels"] for s in samples])
    slide_ids = [s["slide_id"] for s in samples]

    pad_imgs1, pad_coords1, pad_mask1 = pad_tensors(imgs1, coords1)
    pad_imgs2, pad_coords2, pad_mask2 = pad_tensors(imgs2, coords2)
